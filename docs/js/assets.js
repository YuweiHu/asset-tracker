/* ===== 資產追蹤模組：渲染、表單、登入/同步、事件、啟動 =====
 * 由 app.js 在開機時呼叫 initAssets() 啟用。本模組只操作 #view-assets 內的元素。
 */
import {
  CATS,
  CAT_LABEL,
...
// 供 router 在切回資產視圖時重新校正圖表尺寸（避免從 display:none 切回尺寸跑掉）
export function resizeCharts() {
  [allocChart, twChart, usChart, trendChart].forEach((c) => c && c.resize());
}
...
export function initAssets() {
  ...
  el.chartCarousel.addEventListener("pointerdown", (e) => {
    ...
    if (el.chartCarousel.scrollWidth <= el.chartCarousel.clientWidth) return; // 桌面並排時不可捲動
    dragX = e.clientX;
  ...
  window.addEventListener("pointerup", () => {
    ...
    if (dragged)
      scrollToSlide(
        Math.round(el.chartCarousel.scrollLeft / (el.chartCarousel.clientWidth || 1)),
      );
  });
  el.chartCarousel.addEventListener(
    "click",
    (e) => {
      if (dragged) {
        e.stopPropagation();
